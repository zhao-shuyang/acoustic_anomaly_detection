...
def generate_predictions(machine_class, machine_id):
    #Different number of MFCCs for stationary sounds and transient sounds
    ...
    for k in h5_test.keys():
        if machine_id in k:
            ...
            if machine_class in {'fan', 'ToyConveyor', 'ToyCar', 'pump'}:
            ...
            for i in range(n):
                if machine_class in {'fan', 'ToyConveyor', 'ToyCar', 'pump'}:
                    qm = record_emb_means[i][:n_mfccs]
                    qv = record_emb_vars[i][:n_mfccs]
                else:
                ...
                #print (pm.shape, pv.shape,qm.shape, qv.shape, dist_vec[i])
